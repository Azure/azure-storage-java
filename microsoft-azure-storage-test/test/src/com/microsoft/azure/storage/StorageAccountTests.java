...
public class StorageAccountTests {
    ...
    public void testCloudStorageAccountClientMethods() throws URISyntaxException {
        ...
        CloudBlobClient blob = CloudBlobClient.createCloudBlobClient(account);
        CloudQueueClient queue = CloudQueueClient.createCloudQueueClient(account);
        CloudTableClient table = CloudTableClient.createCloudTableClient(account);
        CloudFileClient file = CloudFileClient.createCloudFileClient(account);
    ...
    public void testCloudStorageAccountClientUriVerify() throws URISyntaxException, StorageException {
        ...
        CloudBlobClient blobClient = CloudBlobClient.createCloudBlobClient(cloudStorageAccount);
        CloudBlobContainer container = blobClient.getContainerReference("container1");
        assertEquals(cloudStorageAccount.getBlobEndpoint().toString() + "/container1", container.getUri().toString());

        CloudQueueClient queueClient = CloudQueueClient.createCloudQueueClient(cloudStorageAccount);
        CloudQueue queue = queueClient.getQueueReference("queue1");
        assertEquals(cloudStorageAccount.getQueueEndpoint().toString() + "/queue1", queue.getUri().toString());

        CloudTableClient tableClient = CloudTableClient.createCloudTableClient(cloudStorageAccount);
        CloudTable table = tableClient.getTableReference("table1");
        assertEquals(cloudStorageAccount.getTableEndpoint().toString() + "/table1", table.getUri().toString());

        CloudFileClient fileClient = CloudFileClient.createCloudFileClient(cloudStorageAccount);
    ...
    public void testCloudStorageAccountEndpointSuffix()
        ...
        CloudTableClient tableClientParse = CloudTableClient.createCloudTableClient(accountParse);
